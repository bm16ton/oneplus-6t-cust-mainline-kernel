...
//int ath10k_reboot(struct notifier_block *nb,
...
//	ath10k_snoc_release_resource(ar);
	...
	printk("ath10k system reboot called \n");
	return NOTIFY_DONE;
}

static struct notifier_block reboot_handler = {
	.notifier_call = ath10k_reboot,
};


static int ath10k_reboot_notifier(struct device *dev)
{
	static struct notifier_block reboot_handler;
//	reboot_handler.notifier_call = ath10k_reboot;
//	reboot_handler.priority = 192;
	printk("reboot notifier ath10k\n");
	return devm_register_reboot_notifier(dev, &reboot_handler);
}
*/
...
/*
	ret = ath10k_reboot_notifier(dev);
		if (ret) {
			ath10k_warn(ar, "to set restart func\n");
		}
*/
...
//unregister_reboot_notifier(&reboot_handler);
	ath10k_snoc_free_resources(ar);
	printk("ath10k_snoc removed\n");
}
