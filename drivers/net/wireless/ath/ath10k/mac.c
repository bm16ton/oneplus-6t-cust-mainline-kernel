			      ...
			      const u8 *macaddr, u32 flags)
{
	...
	unsigned long time_left;
     printk("ath10k ath10k_install_key called\n");
	...
	if (time_left == 0)
		return -ETIMEDOUT;
printk("ath10k ath10k_install_key end\n");
	return 0;
}
				  ...
				  const u8 *addr)
{
	...
	int i;
	u32 flags = 0;
     printk("ath10k ath10k_clear_peer_keys called\n");
	lockdep_assert_held(&ar->conf_mutex);
...
printk("ath10k ath10k_clear_peer_keys end\n");
	return first_errno;
}
				 ...
				 struct ieee80211_key_conf *key)
{
	...
	int i;
	u32 flags = 0;
     printk("ath10k ath10k_clear_vdev_key called\n");
	lockdep_assert_held(&ar->conf_mutex);
...
printk("ath10k ath10k_clear_vdev_key end\n");
	return first_errno;
}
...
//		return;
//	}
     printk("ath10k ath10k_wait_for_peer_delete_done called\n");
	if (test_bit(WMI_SERVICE_SYNC_DELETE_CMDS, ar->wmi.svc_map) &&
	 ...
	 printk("ath10k ath10k_wait_for_peer_delete_done end\n");
}
			      ...
			      enum wmi_peer_type peer_type)
{
	struct ath10k_peer *peer;
	int ret;
     printk("ath10k ath10k_peer_create called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	ar->num_peers++;
     printk("ath10k ath10k_peer_create end\n");
	return 0;
}
...
static int ath10k_mac_set_kickout(struct ath10k_vif *arvif)
{
	...
	u32 param;
	int ret;
     printk("ath10k ath10k_mac_set_kickout called\n");
	...
	if (ret) {
     ...
     printk("ath10k ath10k_mac_set_kickout end\n");
	return 0;
}
...
{
	int ret;
     printk("ath10k ath10k_peer_delete called\n");
	lockdep_assert_held(&ar->conf_mutex);
...
//	}
	...
	ar->num_peers--;
     printk("ath10k ath10k_peer_delete end\n");
	return 0;
}
...
{
	int peer_id, i;
     printk("ath10k ath10k_peer_map_cleanup called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	list_del(&peer->list);
	kfree(peer);
	printk("ath10k ath10k_peer_map_cleanup end\n");
	ar->num_peers--;
}
...
static void ath10k_peer_cleanup(struct ath10k *ar, u32 vdev_id)
{
	struct ath10k_peer *peer, *tmp;
     printk("ath10k ath10k_peer_cleanup called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	spin_unlock_bh(&ar->data_lock);
	printk("ath10k ath10k_peer_cleanup end\n");
}
...
static void ath10k_peer_cleanup_all(struct ath10k *ar)
{
	struct ath10k_peer *peer, *tmp;
	int i;
     printk("ath10k ath10k_peer_cleanup_all called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	ar->num_peers = 0;
	ar->num_stations = 0;
	printk("ath10k ath10k_peer_cleanup_all end\n");
}
				       ...
				       enum wmi_tdls_peer_state state)
{
	...
	struct wmi_channel_arg chan_arg = {};
     printk("ath10k ath10k_mac_tdls_peer_update called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	if (ret) {
     ...
     printk("ath10k ath10k_mac_tdls_peer_update end\n");
	return 0;
}
...
{
	struct ath10k *ar = arvif->ar;
     printk("ath10k ath10k_mac_vif_beacon_free called\n");
	lockdep_assert_held(&ar->data_lock);
	...
	arvif->beacon_state = ATH10K_BEACON_SCHEDULED;
	     printk("ath10k ath10k_mac_vif_beacon_free end\n");
}

static void ath10k_mac_vif_beacon_cleanup(struct ath10k_vif *arvif)
{
	struct ath10k *ar = arvif->ar;
     printk("ath10k ath10k_mac_vif_beacon_cleanup called\n");
	lockdep_assert_held(&ar->data_lock);
	...
	if (arvif->beacon_buf) {
		...
		arvif->beacon_buf = NULL;
	     ...
	     printk("ath10k ath10k_mac_vif_beacon_cleanup end\n");
}

static inline int ath10k_vdev_setup_sync(struct ath10k *ar)
{
	unsigned long time_left;
     printk("ath10k ath10k_vdev_setup_sync called\n");
 //    if (poo == 1) {
 //    return -ESHUTDOWN;
 //    }
	...
	if (test_bit(ATH10K_SNOC_FLAG_MODEM_STOPPED, &ar->dev_flags)) {
		return -ESHUTDOWN;
	}
	...
	if (time_left == 0)
		return -ETIMEDOUT;
     printk("ath10k ath10k_vdev_setup_sync end\n");
...
static int ath10k_monitor_vdev_start(struct ath10k *ar, int vdev_id)
{
	...
	int ret = 0;
     printk("ath10k ath10k_monitor_vdev_start called\n");
	lockdep_assert_held(&ar->conf_mutex);
...
vdev_stop:
	...
	if (ret)
     ...
     printk("ath10k ath10k_monitor_vdev_start end\n");
	return ret;
}
...
{
	int ret = 0;
     printk("ath10k ath10k_monitor_vdev_stop called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	ath10k_dbg(ar, ATH10K_DBG_MAC, "mac monitor vdev %i stopped\n",
		   ar->monitor_vdev_id);
		        printk("ath10k ath10k_monitor_vdev_stop end\n");
	return ret;
}
...
{
	int bit, ret = 0;
     printk("ath10k ath10k_monitor_vdev_create called\n");
	lockdep_assert_held(&ar->conf_mutex);
...
{
	int ret = 0;
     printk("ath10k ath10k_monitor_vdev_delete called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	printk("ath10k ath10k_monitor_vdev_delete end\n");
...
{
	int ret;
     printk("ath10k ath10k_monitor_start called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	ath10k_dbg(ar, ATH10K_DBG_MAC, "mac monitor started\n");
     printk("ath10k ath10k_monitor_start end\n");
	return 0;
}
...
{
	int ret;
     printk("ath10k ath10k_monitor_stop called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	ath10k_dbg(ar, ATH10K_DBG_MAC, "mac monitor stopped\n");
     printk("ath10k ath10k_monitor_stop end\n");
	return 0;
}
...
{
	int num_ctx;
     printk("ath10k ath10k_mac_monitor_vdev_is_needed called\n");
	...
	return ar->monitor ||
		...
		(ar->filter_flags & (FIF_OTHER_BSS | FIF_MCAST_ACTION))) ||
	     ...
	     printk("ath10k ath10k_mac_monitor_vdev_is_needed end\n");
}

static bool ath10k_mac_monitor_vdev_is_allowed(struct ath10k *ar)
{
	int num_ctx;
     printk("ath10k ath10k_mac_monitor_vdev_is_allowed called\n");
	num_ctx = ath10k_mac_num_chanctxs(ar);
	...
	if (test_bit(ATH10K_CAC_RUNNING, &ar->dev_flags) && num_ctx > 1)
		return false;
     printk("ath10k ath10k_mac_monitor_vdev_is_allowed end\n");
	return true;
}
...
static int ath10k_monitor_recalc(struct ath10k *ar)
{
	...
	bool allowed;
	int ret;
     printk("ath10k ath10k_monitor_recalc called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	if (WARN_ON(needed && !allowed)) {
     ...
     printk("ath10k ath10k_monitor_recalc end\n");
...
{
	struct ath10k *ar = arvif->ar;
     printk("ath10k ath10k_mac_can_set_cts_prot called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	if (!arvif->is_started) {
     ...
     printk("ath10k ath10k_mac_can_set_cts_prot end\n");
	return true;
}
...
static int ath10k_mac_set_cts_prot(struct ath10k_vif *arvif)
{
	struct ath10k *ar = arvif->ar;
	u32 vdev_param;
     printk("ath10k ath10k_mac_set_cts_prot called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	ath10k_dbg(ar, ATH10K_DBG_MAC, "mac vdev %d cts_protection %d\n",
     ...
     printk("ath10k ath10k_mac_set_cts_prot end\n");
...
static int ath10k_recalc_rtscts_prot(struct ath10k_vif *arvif)
{
	...
	u32 vdev_param, rts_cts = 0;
     printk("ath10k ath10k_recalc_rtscts_prot called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	return ath10k_wmi_vdev_set_param(ar, arvif->vdev_id, vdev_param,
					 rts_cts);
	     printk("ath10k ath10k_recalc_rtscts_prot end\n");
}

static int ath10k_start_cac(struct ath10k *ar)
{
	int ret;
     printk("ath10k ath10k_start_cac called\n");
	lockdep_assert_held(&ar->conf_mutex);
...
{
     printk("ath10k ath10k_stop_cac called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	ath10k_dbg(ar, ATH10K_DBG_MAC, "mac cac finished\n");
     printk("ath10k ath10k_stop_cac end\n");
	return 0;
}
...
static int ath10k_vdev_stop(struct ath10k_vif *arvif)
{
	struct ath10k *ar = arvif->ar;
	int ret;
     printk("ath10k ath10k_vdev_stop called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	if (ar->num_started_vdevs != 0) {
     ...
     printk("ath10k ath10k_vdev_stop end\n");
	return ret;
}
				     ...
				     bool restart)
{
	...
	struct wmi_vdev_start_request_arg arg = {};
	int ret = 0;
     printk("ath10k ath10k_vdev_start_restart called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	if (restart) {
	     printk("ath10k ath10k_vdev_start_restart restart\n");
     	ret = ath10k_wmi_vdev_restart(ar, &arg);
	} else {
	     printk("ath10k ath10k_vdev_start_restart start\n");
		ret = ath10k_wmi_vdev_start(ar, &arg);
	}
	if (ret) {
	...
	ar->num_started_vdevs++;
	ath10k_recalc_radar_detection(ar);
     printk("ath10k ath10k_vdev_start_restart end\n");
	return ret;
}
				       ...
				       u8 oui_type, size_t ie_offset)
{
	...
	const u8 *end;
	u8 *ie;
     printk("ath10k ath10k_mac_remove_vendor_ie called\n");
	...
	skb_trim(skb, skb->len - len);
     printk("ath10k ath10k_mac_remove_vendor_ie end\n");
	return 0;
}
...
static int ath10k_mac_setup_bcn_tmpl(struct ath10k_vif *arvif)
{
	...
	struct sk_buff *bcn;
	int ret;
     printk("ath10k ath10k_mac_setup_bcn_tmpl called\n");
	...
	if (ret) {
     ...
     printk("ath10k ath10k_mac_setup_bcn_tmpl end\n");
	return 0;
}
...
static int ath10k_mac_setup_prb_tmpl(struct ath10k_vif *arvif)
{
	...
	struct sk_buff *prb;
	int ret;
     printk("ath10k ath10k_mac_setup_prb_tmpl called\n");
	if (!test_bit(WMI_SERVICE_BEACON_OFFLOAD, ar->wmi.svc_map))
		return 0;
...
static int ath10k_mac_vif_fix_hidden_ssid(struct ath10k_vif *arvif)
{
	...
	struct cfg80211_chan_def def;
	int ret;
     printk("ath10k ath10k_mac_vif_fix_hidden_ssid called\n");
				...
				struct ieee80211_vif *vif)
{
	...
	u32 vdev_param;
	int ret = 0;
     printk("ath10k ath10k_control_ibss called\n");
	lockdep_assert_held(&arvif->ar->conf_mutex);
	...
	if (ret)
		ath10k_warn(ar, "failed to set IBSS ATIM for vdev %d: %d\n",
	     ...
	     printk("ath10k ath10k_control_ibss end\n");
}
...
static int ath10k_mac_vif_recalc_ps_wake_threshold(struct ath10k_vif *arvif)
{
	...
	u32 value;
	int ret;
     printk("ath10k ath10k_mac_vif_recalc_ps_wake_threshold called\n");
	lockdep_assert_held(&arvif->ar->conf_mutex);
	...
	if (ret) {
     ...
     printk("ath10k ath10k_mac_vif_recalc_ps_wake_threshold end\n");
	return 0;
}
...
static int ath10k_mac_vif_recalc_ps_poll_count(struct ath10k_vif *arvif)
{
	...
	u32 value;
	int ret;
     printk("ath10k ath10k_mac_vif_recalc_ps_poll_count called\n");
	lockdep_assert_held(&arvif->ar->conf_mutex);
	...
	if (ret) {
     ...
     printk("ath10k ath10k_mac_vif_recalc_ps_poll_count end\n");
	return 0;
}
...
static int ath10k_mac_num_vifs_started(struct ath10k *ar)
{
	...
	list_for_each_entry(arvif, &ar->arvifs, list)
		if (arvif->is_started)
			num++;
     printk("ath10k ath10k_mac_num_vifs_started called %d/ ended \n", num);
	return num;
}
...
static int ath10k_mac_vif_setup_ps(struct ath10k_vif *arvif)
{
	...
	int ps_timeout;
	bool enable_ps;
     printk("ath10k ath10k_mac_vif_setup_ps called\n");
	lockdep_assert_held(&arvif->ar->conf_mutex);
	...
	if (ret) {
     ...
     printk("ath10k ath10k_mac_vif_setup_ps ended\n");
exit:
	return 0;
}
...
static int ath10k_mac_vif_disable_keepalive(struct ath10k_vif *arvif)
{
	...
	int ret;
     printk("ath10k ath10k_mac_vif_disable_keepalive called\n");
	lockdep_assert_held(&arvif->ar->conf_mutex);
	...
	if (ret) {
     ...
     printk("ath10k ath10k_mac_vif_disable_keepalive ended\n");
	return 0;
}
				    ...
				    struct wmi_peer_assoc_complete_arg *arg)
{
	...
	u8 max_nss, vht_mcs;
	int i;
     printk("ath10k ath10k_peer_assoc_h_vht called\n");
	if (WARN_ON(ath10k_mac_vif_chan(vif, &def)))
		return;
	...
	ath10k_dbg(ar, ATH10K_DBG_MAC,
	     ...
	     printk("ath10k ath10k_peer_assoc_h_vht ended\n");
}
...
{
	struct ath10k_vif *arvif = (void *)vif->drv_priv;
     printk("ath10k ath10k_peer_assoc_h_qos called\n");
	...
	printk("ath10k ath10k_peer_assoc_h_qos ended\n");
}
...
static int ath10k_peer_assoc_prepare(struct ath10k *ar,
				     ...
				     struct wmi_peer_assoc_complete_arg *arg)
{
     printk("ath10k ath10k_peer_assoc_prepare called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	ath10k_peer_assoc_h_qos(ar, vif, sta, arg);
     printk("ath10k ath10k_peer_assoc_prepare ended\n");
	return 0;
}
...
{
	struct ath10k_vif *arvif;
     printk("ath10k ath10k_mac_is_connected called\n");
	list_for_each_entry(arvif, &ar->arvifs, list) {
		if (arvif->is_up && arvif->vdev_type == WMI_VDEV_TYPE_STA)
			return true;
	}
     printk("ath10k ath10k_mac_is_connected ended\n");
	return false;
}
...
static int ath10k_mac_txpower_setup(struct ath10k *ar, int txpower)
{
	...
	bool connected;
     printk("ath10k ath10k_mac_txpower_setup called\n");
	lockdep_assert_held(&ar->conf_mutex);
...
static int ath10k_mac_txpower_recalc(struct ath10k *ar)
{
	struct ath10k_vif *arvif;
	int ret, txpower = -1;
     printk("ath10k ath10k_mac_txpower_recalc called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	if (ret) {
     ...
     printk("ath10k ath10k_mac_txpower_recalc ended\n");
	return 0;
}
				...
				struct ieee80211_vif *vif)
{
	...
	struct ieee80211_sta_vht_cap vht_cap = {};
	int ret;
     printk("ath10k ath10k_bss_disassoc called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	cancel_delayed_work_sync(&arvif->connection_loss_work);
	     printk("ath10k ath10k_bss_disassoc ended\n");
}
				...
				bool reassoc)
{
	...
	struct wmi_peer_assoc_complete_arg peer_arg;
	int ret = 0;
     printk("ath10k ath10k_station_assoc called\n");
	lockdep_assert_held(&ar->conf_mutex);
   ...
   printk("ath10k ath10k_station_assoc ended\n");
				   ...
				   struct ieee80211_sta *sta)
{
	struct ath10k_vif *arvif = (void *)vif->drv_priv;
	int ret = 0;
     printk("ath10k ath10k_station_disassoc called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	if (ret) {
     ...
     printk("ath10k ath10k_station_disassoc ended\n");
	return ret;
}
...
void ath10k_mgmt_over_wmi_tx_purge(struct ath10k *ar)
{
	...
	for (;;) {
		...
		ieee80211_free_txskb(ar->hw, skb);
	     ...
	     printk("ath10k ath10k_mgmt_over_wmi_tx_purge ended\n");
}
...
static void ath10k_mac_txq_unref(struct ath10k *ar, struct ieee80211_txq *txq)
{
	...
	struct sk_buff *msdu;
	int msdu_id;
     printk("ath10k ath10k_mac_txq_unref called\n");
	if (!txq)
		return;
	...
	spin_unlock_bh(&ar->htt.tx_lock);
	printk("ath10k ath10k_mac_txq_unref ended\n");
}
...
void ath10k_mac_tx_push_pending(struct ath10k *ar)
{
	struct ieee80211_hw *hw = ar->hw;
	u32 ac;
//printk("ath10k ath10k_mac_tx_push_pending called\n");
	if (ar->htt.tx_q_state.mode != HTT_TX_MODE_SWITCH_PUSH)
		return;
	...
	rcu_read_unlock();
//printk("ath10k ath10k_mac_tx_push_pending ended\n");
}
...
void ath10k_scan_finish(struct ath10k *ar)
{
	...
	spin_unlock_bh(&ar->data_lock);
	printk("ath10k ath10k_scan_finish ended\n");
}
...
out:
	...
	if (ar->scan.state != ATH10K_SCAN_IDLE)
		__ath10k_scan_finish(ar);
	spin_unlock_bh(&ar->data_lock);
	printk("ath10k ath10k_scan_stop ended\n");
	return ret;
}
...
static void ath10k_scan_abort(struct ath10k *ar)
{
	...
	spin_unlock_bh(&ar->data_lock);
		printk("ath10k ath10k_scan_abort ended\n");
}
...
{
	struct ath10k_vif *arvif;
     printk("ath10k ath10k_halt called\n");
	lockdep_assert_held(&ar->conf_mutex);
	...
	spin_unlock_bh(&ar->data_lock);
	     printk("ath10k ath10k_halt ended\n");
}
...
static int ath10k_start(struct ieee80211_hw *hw)
{
	...
	struct wmi_bb_timing_cfg_arg bb_timing = {0};
     printk("ath10k ath10k_start called\n");
	/*
	...
	mutex_unlock(&ar->conf_mutex);
	     printk("ath10k ath10k_start end\n");
	return 0;
...
static void ath10k_stop(struct ieee80211_hw *hw)
{
	struct ath10k *ar = hw->priv;
	u32 opt;
     printk("ath10k ath10k_stop called\n");
...
//	}
	...
	cancel_delayed_work_sync(&ar->scan.timeout);
	cancel_work_sync(&ar->restart_work);
	     printk("ath10k ath10k_stop ended\n");
}
...
static int ath10k_config_ps(struct ath10k *ar)
{
	struct ath10k_vif *arvif;
	int ret = 0;
     printk("ath10k ath10k_config_ps called\n");
	if (test_bit(ATH10K_FLAG_CRASH_FLUSH, &ar->dev_flags)) {
		return 0;
	}
	...
	list_for_each_entry(arvif, &ar->arvifs, list) {
     ...
     printk("ath10k ath10k_config_ps ended\n");
	return ret;
}
				...
				struct ieee80211_vif *vif)
{
	...
	u32 vdev_param;
	     ...
	     printk("ath10k add_interface called.\n");
	...
	printk("ath10k add_interface end.\n");
				    ...
				    struct ieee80211_vif *vif)
{
	...
	int i;
	     ...
	     printk("ath10k remove_interface called\n");
...
out:
	mutex_unlock(&ar->conf_mutex);
	printk("ath10k remove_interface end\n");
...
void ath10k_mac_wait_tx_complete(struct ath10k *ar)
{
	...
	time_left = wait_event_timeout(ar->htt.empty_tx_wq, ({
			...
			skip = (ar->state == ATH10K_STATE_WEDGED) || test_bit(ATH10K_SNOC_FLAG_MODEM_STOPPED, &ar->dev_flags) ||
				     ...
				     enum ieee80211_reconfig_type reconfig_type)
{
	struct ath10k *ar = hw->priv;
	struct ath10k_vif *arvif;
     printk("ath10k ath10k_reconfig_complete called\n");
...
struct ath10k *ath10k_mac_create(size_t priv_size)
{
	...
	struct ath10k *ar;
     printk("ath10k ath10k_mac_create called\n");
...
void ath10k_mac_destroy(struct ath10k *ar)
{
	struct ieee80211_ops *ops = ar->ops;
     printk("ath10k ath10k_mac_destroy called\n");
...
{
	ieee80211_unregister_hw(ar->hw);
printk("ath10k ath10k_mac_unregister\n");
	if (IS_ENABLED(CONFIG_ATH10K_DFS_CERTIFIED) && ar->dfs_detector)
		ar->dfs_detector->exit(ar->dfs_detector);
