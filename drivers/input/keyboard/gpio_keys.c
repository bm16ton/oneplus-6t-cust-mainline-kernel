...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	if (type == EV_ABS) {
		if (state) {
			sliderpos2 = button->value;
			input_event(input, type, button->code, button->value);
			}
	} else {
	if (*bdata->code == 115) {           //volume up
		if (sliderpos2 == 0) {
			input_report_key(bdata->input, KEY_LEFTCTRL, state);
			input_sync(bdata->input);
		} else if (sliderpos2 == 1) {
			input_report_key(bdata->input, KEY_LEFTCTRL, state);
			input_report_key(bdata->input, KEY_LEFTALT, state);
			input_report_key(bdata->input, KEY_BACKSPACE, state);
			input_sync(bdata->input);
		} else if (sliderpos2 == 2) {
		input_event(input, type, *bdata->code, state);
		}
	} else if (*bdata->code == 114) {		//volume down
		if (sliderpos2 == 0) {
			input_report_key(bdata->input, KEY_LEFTALT, state);
			input_sync(bdata->input);
		} else if (sliderpos2 == 1) {
			input_report_key(bdata->input, KEY_TAB, state);
			input_sync(bdata->input);
		} else if (sliderpos2 == 2) {
			input_event(input, type, *bdata->code, state);
			}
...
static irqreturn_t gpio_keys_irq_isr(int irq, void *dev_id)
{
	...
	printk("gpio_keys_irq_isr code = %d\n", *bdata->code);
	printk("gpio_keys_irq_isr keypressed %d\n", bdata->key_pressed);
	if (!bdata->key_pressed) {
		if (bdata->button->wakeup)
