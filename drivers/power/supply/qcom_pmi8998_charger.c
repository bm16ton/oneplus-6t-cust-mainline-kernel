...
static ssize_t usbmode_show(struct device *dev,
                         struct device_attribute *attr, char *buf)
{
	struct smb2_chip *chip = dev_get_drvdata(dev);
	enum usb_role role;

	role = usb_role_switch_get_role(chip->role_sw);

   return sprintf(buf, "%d", role);
}

static ssize_t usbmode_store(struct device *dev,
...
static DEVICE_ATTR(usbmode, 0660, usbmode_show,
                   usbmode_store);
...
static irqreturn_t smb2_handle_typce_change(int irq, void *data)
{
//	struct smb2_chip *chip = data;

	printk("USB TypeC change IRQ fired\n");
...
static int smb2_probe(struct platform_device *pdev)
{
	...
	chip->dev = &pdev->dev;
	chip->name = pdev->name;
printk("qcom smb2 probe b4 role_sw\n");
	chip->role_sw = usb_role_switch_get(&pdev->dev);
	if (IS_ERR(chip->role_sw))
		printk("qcom smb2 probe role_sw set failed\n");
//		return dev_err_probe(chip->dev, PTR_ERR(chip->role_sw),
//					"failed to get role switch\n");
//	pdphy->role_sw = chip->role_sw;

printk("qcom smb2 probe after role_sw\n");
...
//     return;
	...
	rc = smb2_init_irq(chip, &irq, "type-c-change",
				smb2_handle_typce_change);
	if (rc < 0) {
		printk("Failed to aquire TYPEC change IRQ!\n");
		return rc;
	}
